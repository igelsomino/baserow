...
def test_view_signals_not_collaborative(data_fixture, alternative_per_group_license_service):
    ...
    with patch("baserow.ws.registries.broadcast_to_channel_group") as broadcast:
        ViewHandler().order_views(user=user, table=table, ownership_type="personal", order=[view.id])
        broadcast.delay.assert_not_called()
