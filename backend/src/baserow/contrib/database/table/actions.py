...
class CreateTableActionType(UndoRedoActionType):
    type = "create_table"
    description = (
        _("Create table"),
        _(
            'Table "%(table_name)s" (%(table_id)s) created in database '
...
class DeleteTableActionType(UndoRedoActionType):
    type = "delete_table"
    description = (
        _("Delete table"),
        _(
            'Table "%(table_name)s" (%(table_id)s) deleted from database '
            '"%(database_name)s" (%(database_id)s)'
        ),
    )
...
class OrderTableActionType(UndoRedoActionType):
    type = "order_tables"
    description = (
        _("Order tables"),
        _('Tables order changed in database "%(database_name)s" (%(database_id)s)'),
    )
...
class UpdateTableActionType(UndoRedoActionType):
    type = "update_table"
    description = (
        _("Update table"),
        _(
            "Table (%(table_id)s) name changed from "
            '"%(original_table_name)s" to "%(table_name)s" in database '
            '"%(database_name)s" (%(database_id)s)'
        ),
    )
...
class DuplicateTableActionType(UndoRedoActionType):
    type = "duplicate_table"
    description = (
        _("Duplicate table"),
        _(
            'Table "%(table_name)s" (%(table_id)s) duplicated from '
            '"%(original_table_name)s" (%(original_table_id)s) in database '
            '"%(database_name)s" (%(database_id)s)'
        ),
    )
