...
class UserHandler:
    ...
    def create_user(
        ...
        # to a workspace, and `allow_global_workspace_creation` is enabled, we'll create
        # a workspace for this new user.
    ...
    def cancel_user_deletion(self, user: AbstractUser):
        """
        Cancels a previously scheduled user account deletion. This action sends an email
    ...
    def delete_expired_users_and_related_workspaces_if_last_admin(
        ...
        are the last admin of some workspaces, these workspaces are also deleted.
        An email is sent to confirm the user account deletion. This task is periodically
        executed.
