...
def test_list_applications(
    ...
    with patch(
        ...
        response = api_client.get(
            reverse("api:applications:list", kwargs={"group_id": workspace_1.id}),
    ...
    response = api_client.get(
        reverse("api:applications:list", kwargs={"group_id": workspace_2.id}),
        **{"HTTP_AUTHORIZATION": f"JWT {token}"},
    )
    ...
    url = reverse("api:applications:list", kwargs={"group_id": workspace_1.id})
...
def test_create_application(api_client, data_fixture, group_compat_timebomb):
    ...
    response = api_client.post(
        reverse("api:applications:list", kwargs={"group_id": workspace.id}),
    ...
    url = reverse("api:applications:list", kwargs={"group_id": workspace_2.id})
    response = api_client.get(url)
    assert response.status_code == HTTP_401_UNAUTHORIZED
...
def test_order_applications(api_client, data_fixture, group_compat_timebomb):
    ...
    response = api_client.post(
        reverse("api:applications:order", kwargs={"group_id": workspace_2.id}),
        {"application_ids": []},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    ...
    response = api_client.post(
        reverse("api:applications:order", kwargs={"group_id": workspace_1.id}),
        {"application_ids": [0]},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
